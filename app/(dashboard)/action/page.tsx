    ...
    <>
      <div className='bg-white w-full h-auto p-5 pt-10'>
        ...
        <div className="grid grid-cols-1 gap-3 mt-5">
          {items.filter((item: any) => item.genre_ids.includes(28)).slice(0,1).map((item:any) => {
              ...
              return(
                <div key={item.id} className="">
                  <div className="flex justify-between items-center">
                    <Image className=" h-80 w-full rounded-lg object-cover object-bottom cursor-pointer" src={imageSrc} alt={item.title || "Movie poster"} width={308} height={193} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-5">
         {items.filter((item: any) => item.genre_ids.includes(28)).map((item: any) => {
              const imageSrc = item.poster_path || item.backdrop_path
                ? `${IMAGE_BASE_URL}${item.poster_path || item.backdrop_path}`
                : "https://placehold.co/48x48/eeeeee/888888?text=No+Image";
